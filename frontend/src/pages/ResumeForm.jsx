...
const ResumeForm = () => {
  ...
  const [accentColor, setAccentColor] = useState("#0891b2"); // NAYA STATE: resume ka accent color
  ...
  useEffect(() => {
    ...
    const fetchResume = async () => {
      try {
        ...
        setAccentColor(resume?.accentColor || "#0891b2"); // NAYA: saved accent color load karo
      } catch (err) {
        if (!isMounted) return;
  ...
  const validateStep = useCallback(
    (stepId) => {
      if (stepId === "personal") {
        ...
        // email ka format valid hai ya nahi
        ...
        //  LinkedIn link ka format valid hai ya nahi
        ...
        //  GitHub link (agar diya hai) ka format
        ...
        //  Portfolio link (agar diya hai) ka format
  ...
  const buildPayload = (isComplete = false) => ({
    ...
    skills,
    projects,
    accentColor, //  accentColor ko save-payload mein shamil karo
    isComplete,
  });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* NAYA BLOCK: accent color picker, form ke top pe */}
        <div className="mb-6">
          ...
          <div className="hidden lg:block sticky top-8 self-start">
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm px-8 py-10 max-h-[calc(100vh-4rem)] overflow-y-auto">
              <ResumePreviewContent
                ...
                projects={projects}
                avatarUrl={avatarUrl}
                accentColor={accentColor} //  live preview ko bhi color pass karo
              />
            </div>
          </div>
        </div>
...
//  PhoneField ab react-phone-number-input use karta hai

const PhoneField = ({ value, onChange }) => (
  <div>
    <label className="block text-sm font-medium text-slate-700 mb-1.5">Phone *</label>
    <PhoneInput
      international          // international format use karo (+92...)
      defaultCountry="PK"    // default country Pakistan set kiya
      value={value}
      onChange={(val) => onChange(val || "")}
      className="phone-input-custom" // humara custom CSS class (Step 5 mein add kiya)
  ...
  return (
    <div>
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        ...
        {/*  purane Field-based phone input ki jagah naya international PhoneField */}
